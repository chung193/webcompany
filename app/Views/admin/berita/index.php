...
foreach ($berita as $berita) { ?>
    ...
    echo '-';
			...
			<td><a href="<?= base_url('admin/berita/edit/' . $berita['id_berita']) ?>">
				...
				<small>
					<br><i class="fa fa-eye"></i> Lượt xem: <?= $berita['hits'] ?>
					<br><i class="fa fa-home"></i> Icon: <i class="<?= $berita['icon'] ?>"></i> <?= $berita['icon'] ?>
					<br><i class="fa fa-calendar-check"></i> xuất bản: <?= tanggal_bulan_menit($berita['tanggal_publish']) ?>
					<br><i class="fa fa-calendar"></i> Cập nhật: <?= tanggal_bulan_menit($berita['tanggal']) ?>
				</small>
			</td>
			<td><small>
			...
			<td>
				<a href="<?= base_url('berita/read/' . $berita['slug_berita']) ?>" class="btn btn-info btn-sm" target="_blank"><i class="fa fa-eye"></i> Đọc</a>
